...
async function getBooks() {
    try {
        ...
        console.warn('Failed to fetch books from API:', error);
    }
    // Fallback to localStorage
    return JSON.parse(localStorage.getItem(BOOKS_KEY) || 'null') || [...DEFAULT_BOOKS];
}
...
async function openEditModal(id) {
    const book = (await getBooks()).find(b => b.id === id);
    if (!book) return;
...
async function goTo(page) {
    ...
    if (page==='books')     await renderBooks();
