...
function updateBadge(){
    ...
    .then(data => {
        const total = data.items.reduce((a, c) => a + c.quantity, 0);
...
function normalizeBook(book){
    ...
    return {
        ...book,
        imgUrl: book.imgUrl || book.image_url || book.imageUrl || '',
        oldPrice: book.oldPrice ?? book.old_price ?? book.oldprice ?? null,
        lang: book.lang || book.language || 'EN',
        price: typeof book.price === 'string' ? parseFloat(book.price) : book.price,
        rating: typeof book.rating === 'string' ? parseFloat(book.rating) : book.rating,
        reviews: typeof book.reviews === 'string' ? parseInt(book.reviews, 10) : book.reviews,
    };
}
...
async function findBook(){
    if (window.SERVER_BOOK !== undefined) {
        return normalizeBook(window.SERVER_BOOK);
    }
    const titleParam=getParam('title');
    if(!titleParam) return null;
    const title = decodeURIComponent(titleParam);
    try {
        const response = await fetch(`/api/book/?title=${encodeURIComponent(title)}`);
        if (response.ok) {
    ...
    const all=[...DEFAULT_BOOKS,...adminBooks.filter(b=>!DEFAULT_BOOKS.find(d=>d.title===b.title))].map(normalizeBook);
...
function renderBook(book){
    ...
    const isSaved=getMyList().some(b=>b.title===data.title);
    const price=parseFloat(data.price)||0;
    const oldPriceVal=data.oldPrice!==null && data.oldPrice!==undefined ? parseFloat(data.oldPrice) : null;
    const discount=parseFloat(data.discount)||0;
    const rating=parseFloat(data.rating)||4.5;
    const reviews=parseInt(data.reviews,10)||0;
    const finalPrice=discount>0?price*(1-discount/100):price;
    const displayPrice=finalPrice||price;

    const badgeClass={Bestseller:'badge-gold',New:'badge-new',Sale:'badge-sale',Classic:'badge-classic'}[data.badge]||'badge-gold';
    const badgeLabel=discount>0?`Sale -${discount}%`:(data.badge||'New');
    const coverSrc=data.imgUrl;
    const lang=data.lang||'EN';

    const coverHTML=coverSrc
        ?`<img class="cover-img" src="${coverSrc}" alt="${data.title}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
       <div class="cover-fallback" style="display:none;background:${data.color||'linear-gradient(145deg,#2d3142,#4f5d75)'}">
         <span class="cover-fallback-text">${data.title}</span>
       </div>`
        :`<div class="cover-fallback" style="background:${data.color||'linear-gradient(145deg,#2d3142,#4f5d75)'}">
        <span class="cover-fallback-text">${data.title}</span>
      </div>`;

    const oldPriceHTML=oldPriceVal?`<span class="price-old">$${oldPriceVal.toFixed(2)}</span>`:'';

    root.innerHTML=`
  <div class="detail-grid">
 
    <div class="cover-col">
      <div class="detail-cover" id="detail-cover-el" style="background:${data.color||'#12121e'}">
        <div class="cover-spine"></div>
        ${coverHTML}
        <div class="cover-author-label">${data.author}</div>
      </div>
    ...
    <div class="info-col">
      ...
      <div class="detail-author">by ${data.author}</div>
      <div class="rating-row">
        <span class="stars">${starsHTML(rating)}</span>
        <span class="rating-count">${rating.toFixed(1)} · ${reviews.toLocaleString()} reviews</span>
      </div>
      <div class="price-row">
        ${oldPriceHTML}
        <span class="price-new">$${displayPrice.toFixed(2)}</span>
      </div>
      <p class="detail-desc">${data.description||'A great book waiting to be discovered.'}</p>
      <div class="cta-row">
      ...
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-val">${data.pages||'—'}</div>
          <div class="meta-key">Pages</div>
        </div>
        <div class="meta-item">
          <div class="meta-val">${data.year||'—'}</div>
          <div class="meta-key">Published</div>
        </div>
        <div class="meta-item">
          <div class="meta-val">${lang}</div>
  ...
  <div class="tab-content active" id="tab-desc">
    <p class="tab-desc-text">${data.full_desc||data.description||''}</p>
  </div>
  ...
  `;
    ...
    document.querySelectorAll('.tab-btn').forEach(btn=>{
        btn.addEventListener('click',()=>{
            ...
            if(btn.dataset.tab==='related') renderRelated(data);
        });
    });

    /* Wire cart button */
    document.getElementById('cart-btn-main').addEventListener('click',()=>addToCart(data,displayPrice));

    /* Wire save/list buttons */
    document.getElementById('save-btn').addEventListener('click',()=>toggleSave(data));
    document.getElementById('list-btn').addEventListener('click',()=>toggleSave(data));
    ...
    document.title=`FOLIO — ${data.title}`;
    updateBadge();
}
...
function renderRelated(currentBook){
    ...
    related.forEach(b=>{
      ...
      <div class="related-info">
        ...
        card.addEventListener('click',()=>{location.href=`/book-details/?title=${encodeURIComponent(b.title)}`});
    ...
    if (book) {
        ...
        if (root) {
            ...
            <div class="not-found">
              ...
              <p>We couldn't find this book. <a href="/books/" style="color:var(--gold)">Browse all books</a></p>
            </div>`;
        }
    }
})();